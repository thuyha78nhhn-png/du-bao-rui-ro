...
# --------------------------------------------------------------------------------
# KHỐI XỬ LÝ HUẤN LUYỆN (Lưu trữ trạng thái vào st.session_state)
# --------------------------------------------------------------------------------
if train_clicked:
    with st.spinner("🔄 Đang xử lý tiền dữ liệu và huấn luyện mô hình..."):
        ...
        # Tiền xử lý dữ liệu (Scaler) giống chuẩn quy trình trong Notebook
    ...
    if 'trained_model' not in st.session_state:
        ...
        if predict_mode == "Nhập chỉ số trực tiếp của một giao dịch":
            ...
            with st.form("single_prediction_form"):
                ...
                # Chia thành các hàng cột để form cân đối
            ...
            if submit_predict:
                ...
                # Áp dụng bộ chuẩn hóa scaler từ lúc huấn luyện (RẤT QUAN TRỌNG)
        ...
        # ------------------------------------------------------------------------
            ...
            st.caption("Yêu cầu: Tệp cấu trúc phải chứa đầy đủ các cột thuộc tính đặc trưng từ `X_1` đến `X_14` (Ví dụ như file dữ liệu `X_new.xlsx` của bạn).")
            ...
            if bulk_file is not None:
                ...
                if bulk_df is not None:
                    # Kiểm tra kiểm tra schema xem có đủ các cột X không
                    ...
                    if missing_cols:
                        ...
                        # Thực hiện tiền xử lý đồng bộ dữ liệu bằng bộ scaler cũ
                        bulk_X = bulk_df[feature_cols]
                        bulk_scaled = scaler.transform(bulk_X)
                        
                        # Dự đoán nhãn rủi ro và xác suất
                        ...
                        # Tạo nút xuất dữ liệu kết quả ra file Excel/CSV để tải về máy
